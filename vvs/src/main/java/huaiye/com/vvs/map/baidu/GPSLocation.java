...
public class GPSLocation {
    ...
    public static GPSLocation get() {
        if (GPS == null) {
            synchronized (GPSLocation.class) {
                if (GPS == null) {
                    GPS = new GPSLocation(MCApp.getInstance());
                }
            }
        }
        return GPS;
    ...
    public boolean isPermissionGranted(){
        if (ActivityCompat.checkSelfPermission(MCApp.getInstance(), Manifest.permission.ACCESS_FINE_LOCATION)
                != PackageManager.PERMISSION_GRANTED || ActivityCompat.checkSelfPermission(MCApp.getInstance(), Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
    ...
    public void startLocation(){
        //需要定位可以运行,才启动GPS定位
        if (!MCApp.getInstance().locationService.isStart() || MCApp.getInstance().locationService.isPause()){
            return;
        }
        if (!isPermissionGranted()){
            return;
        ...
        locationListener = new LocationListener() {
            ...
            public void onLocationChanged(Location location) {
                ...
                MCApp.getInstance().locationService.sendLocation(bdLocation);
            }
