package huaiye.com.vvs.map.baidu;
...
import huaiye.com.vvs.models.map.bean.GPSMapBean;
...
public abstract class LocationStrategy extends BDAbstractLocationListener {
     ...
     */
    private HashMap<Long, BDLocation> lastUploadLocations = new HashMap<>();
    /**
     * 最后一次上传的时间
     */
    ...
    public BDLocation getLastUploadLocations() {
        Long id = AppDatas.Auth().getUserID();
    ...
    public void checkNeedChangeTime(BDLocation currentLocation) {
        ...
        if (currentLocation.getSpeed() >= 8) {
            MCApp.getInstance().locationService.updateTimeInterval(LocationService.DEFAULT_LOCATION_TIME_INTERVAL_HIGH);
        } else {
            MCApp.getInstance().locationService.updateTimeInterval(LocationService.DEFAULT_LOCATION_TIME_INTERVAL);
...
//        millions = System.currentTimeMillis();
    ...
    protected LatLng convertBaiduToGPS(LatLng sourceLatLng) {
            ...
            disposable = io.reactivex.Observable.timer(time, TimeUnit.SECONDS)
                    .subscribe(new Consumer<Long>() {
                        ...
                        public void accept(Long aLong) throws Exception {
                            //定位服务启动了,且正在暂停
                            if (MCApp.getInstance().locationService.isPause() && MCApp.getInstance().locationService.isStart()) {
                                MCApp.getInstance().locationService.resume();
                            }
                        }
                    });
            MCApp.getInstance().locationService.pause();
        }
    }
    ...
    public void onReceiveLocation(BDLocation bdLocation) {
        ...
        MCApp.getInstance().locationService.requestHotSpotState();
